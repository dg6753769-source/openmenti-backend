...
module Engine =
    ...
    let evaluateDestinyAt (evaluationInstant: DateTime) (metrics: UserMetrics) : DestinyReading =
        ...
        | [] ->
            let moon = Astronomy.moonNakshatraAt metrics.BirthData.UtcBirthInstant
            if moon.DistanceToNearestBoundary < NakshatraBoundaryToleranceDegrees then
                ...
                let timeline = Vimshottari.buildTimeline metrics.BirthData.UtcBirthInstant moon
                let events = RuleMatrix.run evaluationInstant metrics timeline
                if List.isEmpty events then
                    InsufficientDataForExactPrediction
                        [ InsufficiencyReason.NoClassicalRuleAlignment(List.length RuleMatrix.rules) ]
                else
                    ExactTimeline(moon, events |> List.sortBy (fun e -> e.Period.RangeStart))
